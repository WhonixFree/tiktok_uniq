...
## 7. Pixel replacement + Gaussian shift (обязательный блок)
Для каждого кадра в выбранной области применяется комбинированная микро-модификация (всегда вместе):
1. очень слабый Gaussian blur (почти у порога заметности);
2. микро-замена пикселей (neighbor duplication), как базовый механизм локальной диверсификации.

Требования к интенсивности:
- blur должен быть минимальным и визуально почти незаметным;
- процент подмены пикселей должен быть малым (в нижнем безопасном диапазоне), чтобы не давать заметных артефактов.

Ограничения применения:
- позиции пикселей для подмены выбираются случайно;
- обработка применяется не ко всей картинке, а только к выбранной области.
...
3. Pixel replacement блок:
   - параметры очень слабого Gaussian blur (минимальная интенсивность),
   - диапазоны малого процента замен,
   - mode (`edge`/`smart`),
