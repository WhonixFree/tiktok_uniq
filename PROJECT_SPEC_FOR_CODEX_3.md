...
## 6.6 Global Temporal Coordination Layer (v2)
All duration-dependent n-insertion temporal effects MUST participate in a single deterministic coordination layer before recipe finalization. This layer is mandatory and does not replace the individual effect planners; planners still generate candidate events, then submit those candidates for global acceptance.

Participating effect families:
1. audio micro speed perturbations;
2. audio freeze-repeat events;
3. video micro speed perturbations;
4. video freeze events;
5. video replace events.

Canonical temporal domain:
- seconds (`float64`) are the global representation for spacing and conflict checks;
- frame-based events (`video freeze`, `video replace`) MUST be converted to seconds via fps for coordination;
- frame precision MUST be preserved for frame-exact events, and accepted freeze/replace events remain exactly one frame.

Each candidate registration MUST include:
- effect type: `AudioMicro`, `AudioFreeze`, `VideoMicro`, `VideoFreeze`, `VideoReplace`;
- start/end time in seconds;
- hardness: `HARD` for frame-exact visual insertions such as freeze/replace, `SOFT` for speed micro/freezing perturbations that may gracefully degrade;
- minimum distance in seconds;
- deterministic ID / seed lineage.

Global spacing rules:
- `HARD ↔ HARD` events MUST NOT overlap;
- `HARD ↔ SOFT` events MUST NOT overlap or violate the configured minimum distance;
- `SOFT ↔ SOFT` events MUST NOT overlap or violate the configured minimum distance unless an effect explicitly documents an exception;
- minimum distance constraints apply ACROSS effect families, not only within a single feature.

Conflict resolution and degradation:
- coordination MUST be deterministic for the same seed and input media;
- runtime randomness is forbidden;
- earlier accepted events are preferred;
- later conflicting events may be shifted, dropped, or reduced in effective count;
- if constraints cannot be satisfied, effective counts are reduced rather than violating `HARD` constraints;
- requested/effective/dropped counts and drop or shift reason codes MUST be recorded in the recipe.
...
2. Temporal-параметры:
   ...
   - video freeze events,
   - replace events,
   - unified `temporal_events` summary with effect type, start/end seconds, hardness, deterministic ID/seed lineage,
   - requested/effective/dropped counts and coordination reason codes per temporal effect,
