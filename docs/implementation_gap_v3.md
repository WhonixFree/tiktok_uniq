...
| 3.1.6 Stream overlay | partial | Overlay discovery and deterministic selection are planned in recipe, but runtime filtergraph overlays a synthetic color source, not media from `--stream-overlay-dir`. | Use selected overlay asset as FFmpeg input and apply stream overlay from discovered recursive assets. |
...
| 3.1.9 Freeze events, exactly 1 frame | partial | Recipe planner generates 1-frame event positions with minimum-distance checks, but render does not apply freeze frames. | Implement one-frame freeze realization in temporal render stage; add integration assertions on graph/output. |
...
| 11. Recommended effect order | partial | Pipeline and tests enforce geometry/color → weak blur → pixel replacement → temporal speed/replace → overlay → encode/mux; freeze realization is still pending. | Implement freeze in temporal stage without changing order. |
...
1. `partial`: stream overlay runtime is not sourced from discovered overlay assets.
2. `partial`: one-frame freeze events are planned but not rendered.
3. `partial`: pixel replacement is approximate, not true sparse per-pixel neighbor duplication per frame.

These gaps block a strict full PASS against v3 mandatory features and should be prioritized before non-critical refinements.
