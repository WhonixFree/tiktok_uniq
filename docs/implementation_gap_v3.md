...
| 3.1.1 Startup check for dependencies is mandatory | done | `runStartupChecks` verifies `ffmpeg`, `ffprobe`, `exiftool`, `python3` before run. | Keep mandatory checks and expand diagnostics with per-platform install hints if needed. |
...
| 4. Startup check behavior on missing dependencies | done | Mandatory dependency failures stop startup with explicit error. | Maintain fail-fast behavior; optionally aggregate actionable install commands. |
